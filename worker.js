...
import {
  ...
  getJobDetails,
  updateJob,
} from './library/utils/internal-service/databaseOperation.js';
...
async function processJob(jobId) {
  ...
  try {
    ...
    if (
      ...
      !clientMarketplaceConfig?.accessToken ||
      !clientMarketplaceConfig?.shop
    ) {
...
redis.on('error', (err) =>
  logger.log('info', '[SHOPIFY-API-IMPORT] 🚨 Redis (worker) error:', err)
);
